...
def graph_worker(cmd_queue: mp.Queue, screen_width: int = 1920):
    ...
    def _pos_right(fig):
        """Coin bas-gauche 700x550 — ne cache pas les flèches du centre."""
        try:
    ...
    def _update_calib(d):
        ...
        if fc_hist:
            tt, vv = zip(*fc_hist)
            ax.plot(tt, vv, "o-", color="crimson", lw=1.5, ms=5)
            ax.set_xlim(0, max(tt)+0.5)
            mg = max(1,(max(vv)-min(vv))*0.1+1)
            ax.set_ylim(min(vv)-mg, max(vv)+mg)
        ...
        if rr_hist:
            tt, vv = zip(*rr_hist)
            ax.plot(tt, vv, "s-", color="darkorange", lw=1.5, ms=5)
            ax.set_xlim(0, max(tt)+0.5)
            mg = max(1,(max(vv)-min(vv))*0.1+1)
            ax.set_ylim(min(vv)-mg, max(vv)+mg)
    ...
    def _update_game(d):
        ...
        if ppg_filt and all_ts:
            ...
            if fp.max() != fp.min(): ax.set_ylim(fp.min()-10, fp.max()+10)
        ...
        if pzt_filt and all_ts:
            ...
            ax.set_xlim(max(0, t_now_phys-WIN), t_now_phys+0.3)
            if fz.max() != fz.min(): ax.set_ylim(fz.min()-10, fz.max()+10)
        ...
        ax.set_ylim(-0.5, 3.5); ax.set_xlim(t_min, t_now+0.3)
        if key_events:
        ...
        if ic_ts and ic_v:
            ...
            ax.set_xlim(max(0, t_now-WIN), t_now+0.3); ax.set_ylim(-0.5, 2.5)
    ...
    def _show_final(d):
        ...
        CALIB    = d.get("calib_sec", 20.0); THRESHOLD = 1.5
        ...
        fig2.suptitle(
            f"ChunkyMemo — Session complète  |  Mode:{mode}  Niveaux:{n_lvs}  Max:N{max_lv}\n"
            f"{fc_s}  {rr_s}  {ic_s}",
            fontsize=12, fontweight="bold")

        ax1 = fig2.add_subplot(gs2[0,0])
        ax1.set(title="PPG brut — session complète", ylabel="Amplitude"); ax1.grid(True,alpha=0.3)
        if all_ts and all_ppg: ax1.plot(all_ts, all_ppg, color="lightcoral", lw=0.4, alpha=0.8)
        ...
        if all_ts and all_pzt: ax3.plot(all_ts, all_pzt, color="moccasin", lw=0.4, alpha=0.8)
        ...
        ax5.axvline(x=CALIB, color="gray", ls="--", alpha=0.6, label="Fin calibration")
        ...
        if fc_v_a:
            ...
            for lv in lvs:
                ...
                if lts:
                    ...
                    ax5.text(lts, fc_max*0.98, f"N{lnum}", fontsize=7, color="#014F84", ha="center")
        ...
        ax6.axvline(x=CALIB, color="gray", ls="--", alpha=0.6)
        ...
        if key_ev:
            ...
            ax6b.set_yticks([0,1,2,3]); ax6b.set_yticklabels(["◄","▼","▲","►"],fontsize=10)
        ...
        if rr_v_a:
            ...
            for lv in lvs:
                ...
                if lts:
                    ...
                    ax6.text(lts, rr_max*0.98, f"N{lnum}", fontsize=7, color="#014F84", ha="center")
        ...
        ax7.axvline(x=CALIB, color="gray", ls="--", alpha=0.6, label="Fin calibration")
        ...
        _pos_right(fig2)
        plt.show(block=False); plt.pause(0.1); plt.ion()
