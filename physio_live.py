...
class ChunkyDevice(plux.SignalsDev):
    def __init__(self, address):
        ...
        self.data_queue = None; self.stop_event = None
...
class AcqThread(threading.Thread):
    def __init__(self, q):
        ...
        self.q = q; self.stop_event = threading.Event(); self.device = None
...
def main():
    ...
    for key in ['s','q','z','Z','d','g','p','f','h','l']:
        for km in ['keymap.save','keymap.quit','keymap.zoom','keymap.xscale',
                   'keymap.yscale','keymap.grid','keymap.pan']:
            try:
    ...
    while data_q.empty() and time.time() < deadline:
        time.sleep(0.1)
    if data_q.empty():
        print("ERREUR : aucune donnée BITalino"); sys.exit(1)
    ...
    ax_keys.set(title="Touches jeu — ▲▼◄► (vert=correct  rouge=erreur)",
                ylabel="Direction", xlabel="Temps (s)")
    ...
    ax_icog.set(title="I_cog = (z_FC + z_PWA_inv + z_RR + z_RT) / 4",
    ...
    def update(frame):
        ...
        if _game_ended[0] and _close_at[0] and time.time() >= _close_at[0]:
            if not _closing[0]:
                ...
                _show_final_in_fig(fig, gs,
                                   all_ts, all_ppg, all_pzt,
                                   fc_ts, fc_v, rr_ts, rr_v,
                                   ic_ts, ic_v, ppg, pzt, cog, CALIB_SEC)
            return

        # Vider la queue
        while not data_q.empty():
            try: s = data_q.get_nowait()
            except: break
            ...
            if ts_rel < CALIB_SEC:
                ...
                if ppg.pwa_raw: calib_vals["pwa"].append(ppg.pwa_raw)
                if pzt.rr_rpm:
                ...
                if not calibrated[0]:
                    ...
                    if calib_vals["fc"]:  cog.set_baseline("fc",  calib_vals["fc"])
                    if calib_vals["rr"]:  cog.set_baseline("rr",  calib_vals["rr"])
                ...
                if ppg.fc_bpm: fc_ts.append(ts_rel); fc_v.append(ppg.fc_bpm)
                if pzt.rr_rpm: rr_ts.append(ts_rel); rr_v.append(pzt.rr_rpm)
                if ic is not None: ic_ts.append(ts_rel); ic_v.append(ic)
        ...
        # PPG brut
        ax_ppg_raw.cla(); ax_ppg_raw.grid(True, alpha=0.3)
        ax_ppg_raw.set(title="PPG brut", ylabel="Amplitude")
        ...
        # PPG filtré
        ax_ppg_filt.cla(); ax_ppg_filt.grid(True, alpha=0.3)
        ...
        # PZT filtré
        ax_pzt_filt.cla(); ax_pzt_filt.grid(True, alpha=0.3)
        ...
        try:
            ...
            for k in _ev.get("keys", []):
                ...
                if ts_k >= x_min:
                    ...
                    ax_keys.scatter(ts_k, y, color=c, s=150,
        ...
        st   = "[CALIBRATION — restez immobile]" if not calibrated[0] \
...
def _show_final_in_fig(fig, gs,
    ...
    gs2 = gridspec.GridSpec(5, 2, figure=fig,
                            height_ratios=[2,2,1.5,1.5,1.5],
                            hspace=0.65, wspace=0.35)

    fc_fin = ppg.fc_bpm; rr_fin = pzt.rr_rpm; ic_fin = cog.i_cog
    ...
    if bl_fc: ax5.axhline(y=bl_fc, color="gray", ls=":", lw=1.5, label=f"Baseline {bl_fc:.0f}")
    if fc_v:
        fc_max = max(fc_v)
        for lv in ev.get("levels",[]):
            lts = lv["ts"]
            ax5.axvline(x=lts, color="#014F84", alpha=0.3, lw=1)
            ax5.text(lts, fc_max*0.97, f"N{lv['level']}", fontsize=7,
                     color="#014F84", ha="center")
    ...
    if bl_rr: ax6.axhline(y=bl_rr, color="gray", ls=":", lw=1.5, label=f"Baseline {bl_rr:.0f}")
    keys = ev.get("keys",[])
    if keys:
        ax6b = ax6.twinx()
        ok  = [(k["ts"],DIR_Y.get(k["direction"],2)) for k in keys if k["correct"]]
        err = [(k["ts"],DIR_Y.get(k["direction"],2)) for k in keys if not k["correct"]]
        if ok:  xs,ys=zip(*ok);  ax6b.scatter(xs,ys,color="#1D9E75",s=80,marker="^",alpha=0.8,zorder=5,label="Correct")
        if err: xs,ys=zip(*err); ax6b.scatter(xs,ys,color="#E24B4A",s=80,marker="x",alpha=0.8,zorder=5,linewidths=2,label="Erreur")
        ax6b.set_yticks([0,1,2,3]); ax6b.set_yticklabels(["◄","▼","▲","►"],fontsize=9)
        ax6b.set_ylim(-0.5,3.5); ax6b.legend(fontsize=8,loc="upper right")
    if rr_v:
        rr_max = max(rr_v)
        for lv in ev.get("levels",[]):
            lts = lv["ts"]
            ax6.axvline(x=lts,color="#014F84",alpha=0.3,lw=1)
            ax6.text(lts,rr_max*0.97,f"N{lv['level']}",fontsize=7,color="#014F84",ha="center")
    ...
    if ic_ts and ic_v:
        ...
        ax7.fill_between(ic_ts, ic_v, THRESHOLD,
            where=[v>THRESHOLD for v in ic_v], color="red", alpha=0.25, label="Surcharge")
