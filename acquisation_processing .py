...
def main():
    ...
    CALIB_SEC = 30.0     # duree calibration au repos
    sep = "=" * 60
    print()
    print(sep)
    print("IMPORTANT :")
    print("  1) Attendez 30s de calibration (restez immobile)")
    print("  2) Apres [calibration terminee] : jouez, bougez, respirez")
    print("  3) Fermez la fenetre quand vous avez fini")
    print(sep)
    print()
    calibrated = [False]
    ...
    def update(frame):
        ...
        if len(ic_ts) > 1:
            ...
            if im2:
                ...
                # Supprimer les anciennes zones colorees (compatible toutes versions matplotlib)
                for coll in list(ax_icog.collections):
                    coll.remove()
        ...
        if not calibrated[0]:
            ...
            st  = f" [CALIBRATION {rem:.0f}s — RESTEZ IMMOBILE — NE FERMEZ PAS]"
        elif cog.i_cog is not None:
            ov  = "  *** SURCHARGE ***" if cog.overload else ""
            st  = f" | I_cog={cog.i_cog:.2f}{ov}  — Fermez quand vous voulez"
        else:
            st = "  [calibration terminee — calcul I_cog en cours...]"
        fig.suptitle(f"ChunkyMemo — {fc_s}  {rr_s}{ap_s}{st}", fontsize=10)
